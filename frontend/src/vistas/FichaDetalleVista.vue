  ...
  <v-container>
    ...
    <div v-else>
      ...
      <div class="contenedor-flex">
        ...
        <v-text-field label="Tipo de ficha" v-model="fichaSeleccionada.tipoFicha" readonly
          class="input-field"></v-text-field>
        <v-text-field label="Parte del entrenamiento" v-model="fichaSeleccionada.parteSesion" readonly
          class="input-field"></v-text-field>
      </div>

      <FichaComponent :rondas="fichaSeleccionada.rutina" :solo-lectura="true" />

      <FabBotonComponent v-if="perfil == 'ECEF' && fichaSeleccionada.estado == 'PENDIENTE'"
        :icon="'mdi-thumb-up-outline'" :clases-adicionales="'claro'" @click="dialogAprobar = true">
      </FabBotonComponent>

      <FabBotonComponent v-if="perfil == 'ECEF' && fichaSeleccionada.estado == 'PENDIENTE'"
        :icon="'mdi-thumb-down-outline'" :clases-adicionales="'rechazo'" @click="dialogRechazar = true">
      </FabBotonComponent>
    </div>
    ...
    <v-dialog v-model="dialogAprobar" max-width="500px">
      <v-card>
        ...
        <v-card-text>¿Estás seguro de que quieres aprobar esta ficha?</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialogRechazar" max-width="500px">
      <v-card>
        <v-card-title class="headline">Confirmar Acción</v-card-title>
        <v-card-text>¿Estás seguro de que quieres rechazar esta ficha?</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  components: {
    FichaComponent, FabBotonComponent
  },
  data() {
    return {
