  ...
  <v-card elevation="2" class="ronda-componente contenedor carta">
    <div class="ronda-header">
      <div class="contenedor-input" v-if="!soloLectura">
        ...
        ></v-select>
        <v-text-field
          v-if="ronda.tipo === 'REPS'"
          v-model="ronda.cantidad"
          label="Cantidad"
          type="number"
          required
      ...
      <div v-else class="flex-fila">
        <p class="definicion-ronda" v-if="ronda.tipo == 'REPS'">
          <b>{{ ronda.cantidad }} repeticiones</b>
        </p>
        <p class="definicion-ronda" v-if="ronda.tipo == 'TIEMPO'">
          <b>Ronda de {{ formatoTiempo }} minutos</b>
        </p>
      </div>
...
export default {
  ...
  computed: {
    ...
    formatoTiempo: {
      get() {
        const minutes = Math.floor(this.ronda.cantidad / 60)
          .toString()
          .padStart(2, "0");
  ...
  methods: {
    ...
    quitarSerie(id) {
      if (!this.soloLectura) {
        this.ronda.series = this.ronda.series.filter(
          (serie) => serie.id !== id
        );
      }
    },
    ...
    actualizarSeries(nuevaSerie) {
      if (!this.soloLectura) {
        const index = this.ronda.series.findIndex(
          (serie) => serie.id === nuevaSerie.id
        );
        if (index !== -1) {
